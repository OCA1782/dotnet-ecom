...
PG_IP = '172.18.0.4'; LOCAL_PORT = 15441
...
# Check for -v{n} SKUs
cur.execute("""
    SELECT "SKU", "Name", "CreatedDate"
    FROM "Products"
    WHERE "SKU" LIKE '%-v%' AND "IsDeleted"=false
    ORDER BY "CreatedDate" DESC
    LIMIT 20
""")
rows = cur.fetchall()
print(f"Products with -v{{n}} suffix SKUs: {len(rows)}")
for r in rows:
    sku = r[0]
    name = (r[1] or '')[:70]
...
cur.execute("SELECT COUNT(*) FROM \"Products\" WHERE \"DataSource\"='catalogiq' AND \"IsDeleted\"=false")
print(f"CatalogIQ sourced: {cur.fetchone()[0]:,}")

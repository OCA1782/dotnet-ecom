  ...
  if (!order) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-16 text-center">
        <p className="text-zinc-500">Sipariş bulunamadı</p>
        <Link href="/hesabim/siparisler" className="mt-4 inline-block text-sm underline text-zinc-700">
          Siparişlerime Dön
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <Link href="/hesabim/siparisler" className="text-sm text-zinc-500 hover:text-zinc-800">
            ← Siparişlerim
          </Link>
          <h1 className="text-2xl font-bold text-zinc-900 mt-1">{order.orderNumber}</h1>
          <p className="text-sm text-zinc-500">{formatDate(order.createdDate)}</p>
        </div>
        <div className="text-right">
          <span className="inline-block text-sm px-3 py-1.5 rounded-full bg-zinc-100 text-zinc-800 font-medium">
            {ORDER_STATUS[order.status] ?? "—"}
          </span>
          <p className="text-xs text-zinc-500 mt-1">Ödeme: {PAYMENT_STATUS[order.paymentStatus] ?? "—"}</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
        <div className="px-5 py-3 border-b border-zinc-100">
          <h2 className="font-semibold text-zinc-800">Ürünler</h2>
        </div>
        <div className="divide-y divide-zinc-100">
          {order.items.map((item) => (
            <div key={item.id} className="flex gap-4 p-4">
              <div className="w-14 h-14 bg-zinc-100 rounded-lg flex items-center justify-center shrink-0">
                {item.imageUrl ? (
              ...
              <div className="text-right text-sm shrink-0">
                <p className="font-medium text-zinc-900">{formatPrice(item.lineTotal)}</p>
        ...
        <div className="border-t border-zinc-100 px-5 py-4 space-y-1.5">
      ...
      {Object.keys(shippingAddr).length > 0 && (
        <div className="bg-white border border-zinc-200 rounded-xl p-5">
          ...
          <p className="text-sm text-zinc-500">{shippingAddr.fullAddress}</p>
          <p className="text-sm text-zinc-500">
            {shippingAddr.district}, {shippingAddr.city}
          </p>
          {shippingAddr.phoneNumber && (
            <p className="text-sm text-zinc-500">{shippingAddr.phoneNumber}</p>
          )}
        </div>
      )}
      ...
      {order.statusHistory?.length > 0 && (
        <div className="bg-white border border-zinc-200 rounded-xl p-5">
          <h2 className="font-semibold text-zinc-800 mb-4">Durum Geçmişi</h2>
          <div className="space-y-3">
            {order.statusHistory.map((h, i) => (
              <div key={i} className="flex gap-3 text-sm">
                <span className="text-zinc-400 shrink-0 text-xs mt-0.5">{formatDate(h.changedAt)}</span>
                <span className="text-zinc-600">
                  {ORDER_STATUS[h.fromStatus] ?? "—"} → <span className="font-medium text-zinc-800">{ORDER_STATUS[h.toStatus] ?? "—"}</span>
      ...
      {canCancel && (
        <div className="flex justify-end">
          <button
            onClick={cancelOrder}
            disabled={cancelling}
            className="px-5 py-2.5 border border-red-300 text-red-600 text-sm rounded-lg hover:bg-red-50 transition disabled:opacity-50"
          >

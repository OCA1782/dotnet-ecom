  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-2xl font-bold text-zinc-900">Adreslerim</h1>
        <button
          onClick={openNew}
          className="bg-zinc-900 text-white text-sm font-semibold px-4 py-2.5 rounded-lg hover:bg-zinc-700 transition"
        >
          + Yeni Adres
        </button>
      </div>
      ...
      {addresses.length === 0 && !showForm && (
        <div className="text-center py-16 text-zinc-400">
          ...
          <button onClick={openNew} className="mt-3 text-sm underline text-zinc-600 hover:text-zinc-900">
      ...
      <div className="space-y-4">
        {addresses.map((addr) => (
          <div key={addr.id} className="bg-white border border-zinc-200 rounded-xl p-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <p className="font-semibold text-zinc-900">{addr.addressTitle}</p>
                  {addr.isDefaultShipping && (
                    <span className="text-xs px-2 py-0.5 bg-zinc-100 text-zinc-600 rounded-full">Varsayılan</span>
                  )}
                </div>
                <p className="text-sm text-zinc-700">
                  {addr.firstName} {addr.lastName} · {addr.phoneNumber}
                </p>
                <p className="text-sm text-zinc-500 mt-0.5">{addr.fullAddress}</p>
              ...
              <div className="flex gap-2 shrink-0">
                <button
                  ...
                  className="text-xs px-3 py-1.5 border border-zinc-300 rounded-lg hover:bg-zinc-50 text-zinc-600 transition"
                >
                  Düzenle
                </button>
                <button
                  onClick={() => handleDelete(addr.id)}
                  className="text-xs px-3 py-1.5 border border-red-200 rounded-lg hover:bg-red-50 text-red-500 transition"
                >
                  Sil
                </button>
              </div>
      ...
      {showForm && (
        <div className="mt-6 bg-white border border-zinc-200 rounded-xl p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-1">Adres Başlığı</label>
              <input
                name="addressTitle"
                required
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-1">Açık Adres</label>
              <textarea
                name="fullAddress"
                required
                ...
                onChange={handleChange}
                rows={3}
                className="w-full border border-zinc-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-500 resize-none"
              />
            </div>
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="rounded"
              />
              <label htmlFor="isDefaultShipping" className="text-sm text-zinc-700">

...
public class I18nDictionaryBuilderJob(
    IServiceScopeFactory scopeFactory,
    IWebHostEnvironment env,
    IConfiguration config,
    IMemoryCache cache) : IJobRunner
{
    public string Name => "I18nDictionaryBuilderJob";
    public string Description => "Admin .tsx sayfalarını tarar, güvenli modda i18n anahtar önerileri üretir";
    ...
    public async Task RunAsync(Func<string, Task> log, CancellationToken ct)
    {
        ...
        await log("i18n Dictionary Builder başlıyor...");
        ...
        var i18nPath = Path.Combine(projectRoot, "frontend", "admin", "src", "lib", "i18n.ts");
        ...
        foreach (var file in tsxFiles)
        {
            ...
            if (found > 0)
                fileHits[Path.GetRelativePath(adminPagesDir, file).Replace("\\", "/")] = found;
        }

        if (newStrings.Count == 0)
        {
            await log("  ✅ Yeni string bulunamadı — i18n.ts güncel");
            cache.Set("i18n:builder:lastResult", "no-new-strings", TimeSpan.FromHours(13));
        ...
        cache.Set("i18n:builder:lastAdded", newEntries.Count, TimeSpan.FromHours(13));
        cache.Set("i18n:builder:lastResult", $"added-{newEntries.Count}", TimeSpan.FromHours(13));
        ...
        else
        {
            ...
            await log("  ℹ Kaynak yazımı için I18nJob:AllowSourceMutation=true gerekir");
        }
        ...
        CancellationToken ct)
    {
        ...
        {
            await log("  ℹ Docs yazımı kapalı — I18nJob:AllowDocsWrite=true olmadan I18N_STATUS.md güncellenmedi");
            return;
        }
        ...
        // Read existing content to preserve the header and history
        string existing = "";
        ...
        sb.AppendLine();
        sb.AppendLine($"> Son güncelleme: {DateTime.UtcNow:dd.MM.yyyy HH:mm} UTC (I18nDictionaryBuilderJob otomatik)");
        sb.AppendLine();
    ...
    private bool AllowSourceMutation(IReadOnlyDictionary<string, string> siteSettings) =>
        ResolveBoolSetting(siteSettings, "I18nJob:AllowSourceMutation", false);

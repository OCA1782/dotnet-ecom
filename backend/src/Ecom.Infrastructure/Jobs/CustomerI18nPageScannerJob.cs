...
public class CustomerI18nPageScannerJob(
...
{
    public string Name => "CustomerI18nPageScannerJob";
    public string Description => "Customer ekranlarını i18n kapsamı için tarar, güvenli modda log/cache raporu üretir, gerekirse builder'ı tetikler";
    ...
    public async Task RunAsync(Func<string, Task> log, CancellationToken ct)
    {
        ...
        await log("Customer i18n Page Scanner başlıyor...");
        ...
        {
            await log("  ⚠ Customer tarama dizinleri bulunamadı");
            return;
        }
        ...
        foreach (var file in tsxFiles)
        {
            ...
            results.Add(await ScanFileAsync(file, customerSrcDir, ct));
        }
        ...
        if (docsPath is not null && AllowDocsWrite(siteSettings))
        {
            ...
            var scanFile = Path.Combine(docsPath, $"customer-i18n-scan-{dateStr}.md");
            var changelogFile = Path.Combine(docsPath, "customer-i18n-changelog.md");
            ...
            await log($"  ✓ customer-i18n-scan-{dateStr}.md yazıldı");
            ...
            await log($"  ✓ customer-i18n-changelog.md güncellendi");
        ...
        {
            await log("  ℹ Docs yazımı kapalı — CustomerI18nJob:AllowDocsWrite=true olmadan rapor dosyası üretilmedi");
        }
        else
        {
        ...
        var prevCount = cache.Get<int?>("customer:i18n:scan:hardcodedCount");
        cache.Set("customer:i18n:scan:hardcodedCount", totalHardcoded, TimeSpan.FromDays(2));
        cache.Set("customer:i18n:scan:coveragePct", coveragePct, TimeSpan.FromDays(2));
        ...
        if (totalHardcoded > 0)
        {
            ...
            {
                await log("  ℹ Builder tetikleme kapalı — CustomerI18nJob:TriggerBuilderFromScanner=true olmadan builder kuyruğa alınmadı");
            }
            else try
            {
                stateManager.Trigger("CustomerI18nDictionaryBuilderJob");
                await log("  ✓ CustomerI18nDictionaryBuilderJob tetiklendi (30s içinde çalışacak)");
            }
            catch (Exception ex)
            {
        ...
        {
            await log("  ✅ Tüm customer sayfalar tam çevrilmiş — builder tetiklenmedi");
        }
    }

    private async Task<PageResult> ScanFileAsync(string filePath, string customerSrcDir, CancellationToken ct)
    {
        var rel = Path.GetRelativePath(customerSrcDir, filePath).Replace("\\", "/");
    ...
    private static string BuildScanReport(List<PageResult> results, int coveragePct)
    {
        ...
        sb.AppendLine($"# Customer i18n Tarama Raporu — {DateTime.UtcNow:yyyy-MM-dd HH:mm} UTC");
        sb.AppendLine();
        sb.AppendLine("> Otomatik oluşturuldu: CustomerI18nPageScannerJob");
        sb.AppendLine();
        sb.AppendLine("## Özet");
        sb.AppendLine();
        ...
        CancellationToken ct)
    {
        ...
        sb.AppendLine();
        sb.AppendLine($"> Son güncelleme: {DateTime.UtcNow:dd.MM.yyyy HH:mm} UTC (CustomerI18nPageScannerJob otomatik)");
        sb.AppendLine();
    ...
    private bool AllowBuilderTrigger(IReadOnlyDictionary<string, string> siteSettings) =>
        ResolveBoolSetting(siteSettings, "CustomerI18nJob:TriggerBuilderFromScanner", false);

# Araç kategorisi seed scripti
# Admin > Kategoriler ekranındaki kayıtları oluşturur
# Çalıştır: pwsh tools/seed-vehicle-categories.ps1
...
function Slugify($s) {
    ...
    $s = $s -replace 'Ğ','g' -replace 'Ü','u' -replace 'Ş','s' -replace 'İ','i' -replace 'Ö','o' -replace 'Ç','c'
    $s = $s -replace '[^a-z0-9 -]',''
    $s = $s -replace '\s+','-'
    $s = $s -replace '-+','-'
    return $s.Trim('-')
...
function RunSql($sql) {
    $result = sqlcmd -S $server -d $database -Q $sql -h -1 -W 2>&1
    return $result
}

function RunSqlScalar($sql) {
    $result = sqlcmd -S $server -d $database -Q $sql -h -1 -W 2>&1
    return ($result | Where-Object { $_ -ne "" } | Select-Object -First 1).Trim()
}
...
foreach ($brandName in $data.Keys) {
    $models = $data[$brandName]
    $brandSlug = Slugify $brandName
    $brandId = [guid]::NewGuid().ToString().ToUpper()

    # Slug çakışması varsa atla
    $sqlLines += @"
IF NOT EXISTS (SELECT 1 FROM Categories WHERE Slug = '$brandSlug' AND IsDeleted = 0)
BEGIN
    ...
    VALUES ('$brandId', N'$brandName', '$brandSlug', NULL, NULL, NULL, $brandOrder, 1, 1, 1, 0, '$now', NULL, NULL, NULL, NULL, NULL, NULL, NULL)
END
ELSE
BEGIN
    -- Varsa ShowInVehicleNav=true güncelle
    UPDATE Categories SET ShowInVehicleNav = 1 WHERE Slug = '$brandSlug' AND IsDeleted = 0
END
"@

    $modelOrder = 0
    foreach ($modelName in $models) {
        $modelSlug = "$brandSlug-$(Slugify $modelName)"
        # 200 char sınırı
        if ($modelSlug.Length -gt 190) { $modelSlug = $modelSlug.Substring(0, 190) }
        $modelId = [guid]::NewGuid().ToString().ToUpper()
        $modelNameEscaped = $modelName -replace "'", "''"

        $sqlLines += @"
IF NOT EXISTS (SELECT 1 FROM Categories WHERE Slug = '$modelSlug' AND IsDeleted = 0)
BEGIN
    DECLARE @parentId_$($modelId -replace '-','') uniqueidentifier
    SELECT @parentId_$($modelId -replace '-','') = Id FROM Categories WHERE Slug = '$brandSlug' AND IsDeleted = 0
    IF @parentId_$($modelId -replace '-','') IS NOT NULL
        INSERT INTO Categories (Id, Name, Slug, ParentCategoryId, Description, ImageUrl, SortOrder, IsActive, ShowInMenu, ShowInVehicleNav, IsDeleted, CreatedDate, MetaTitle, MetaDescription, Icon, StylesJson, VideoUrl, ImportedFromSourceId, CreatedByAdminId)
        VALUES ('$modelId', N'$modelNameEscaped', '$modelSlug', @parentId_$($modelId -replace '-',''), NULL, NULL, $modelOrder, 1, 1, 0, 0, '$now', NULL, NULL, NULL, NULL, NULL, NULL, NULL)
END
"@
        $modelOrder++
    }
...
$sqlFile = "$PSScriptRoot\seed-vehicle-categories.sql"
$sqlContent | Out-File -FilePath $sqlFile -Encoding UTF8

Write-Host "SQL dosyasi olusturuldu: $sqlFile" -ForegroundColor Cyan
Write-Host "Toplam marka: $($data.Keys.Count)" -ForegroundColor Cyan

# Calistir
Write-Host "Veritabanina ekleniyor..." -ForegroundColor Yellow
$output = sqlcmd -S $server -d $database -i $sqlFile 2>&1
if ($LASTEXITCODE -eq 0) {
    Write-Host "Basariyla tamamlandi!" -ForegroundColor Green
    $count = (sqlcmd -S $server -d $database -Q "SELECT COUNT(*) FROM Categories WHERE ShowInVehicleNav = 1 AND IsDeleted = 0" -h -1 -W 2>&1 | Where-Object { $_ -match '^\d+$' } | Select-Object -First 1).Trim()
    Write-Host "Arac menusunde gorunen kategori sayisi: $count" -ForegroundColor Green
} else {
    Write-Host "HATA: $output" -ForegroundColor Red
}
